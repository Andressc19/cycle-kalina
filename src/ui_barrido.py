...
def renderizar_panel_barrido(vals: dict, backend_sel: str) -> None:
    ...
    if st.button("Ejecutar barrido", key="btn_ejecutar_barrido"):
        if n_puntos > _TOPE_PUNTOS:
            ...
            if aviso:
                st.warning(aviso)
            with st.spinner(f"Resolviendo {n_puntos} punto(s) con {nombre_backend}..."):
                filas = sensitivity.ejecutar_barrido(backend, variables)
            st.session_state["resultado_barrido"] = sensitivity.tabla_barrido(filas)
            st.session_state["backend_barrido"] = nombre_backend
